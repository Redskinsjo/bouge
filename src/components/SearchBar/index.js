    ...
    const fetchDataCity = async (source) => {
        try {
            const { data } = await axios.get(
                "https://bouge-api.herokuapp.com/v1.0/city/ids",
                {
                    cancelToken: source.token,
                }
            );
            if (data) {
                const dataMapped = data.data.map((city) => {
    ...
    const fetchDataSport = async (source) => {
        try {
            const response = await axios.get(
                "https://bouge-api.herokuapp.com/v1.0/activity",
                {
                    cancelToken: source.token,
                }
            );
            if (response) {
                const dataMapped = response.data[0].map((sport) => {
                    return { sport: sport.macro_activity_name, type: "sport" };
                });
                setSportData(dataMapped);
                // setIsLoading(false);
                // console.log("setSportData");
            } else {
